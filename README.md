...
3. ~~**Ternary learning is starved by saturation**~~ → **fixed by the toggle rule** (#8–#10): 94–100% no-op flips on ±1-saturated weights disappear; toggled runs keep diversity and reach 6.43 held-out CE. Left to verify: whether toggle stays stable past 1000+ steps (the mass-kick risk at the accumulator wall when starting from non-zeroed state, #10).
4. **The ternary substrate learns, but its on-device headroom is small**: cut-the-tail shows the ternary learned during Python training is worth ~1 nat over random, but continued C++ flips add only ~0.008 nats (only 0.18% of weights move). Toggle raises that to ~0.16 nats on-device (6.996 → 6.838); the co-adapted (embedding + ternary) mechanism still has untested scaling.
