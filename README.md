...
12. **Toggle long-run stability — the mass-kick risk (#10) is confirmed, and toggle is NOT stable past ~1000 steps (open question #3, now answered)**: a 2000-block on-device continuation from the toggle checkpoint mass-kicks at the very first flip pass (block 25) **even from zeroed accumulators** (0 flips in the first 4 passes, then 1.26M/1.75M/1.39M in passes 5–7). CE stays ~6.8 through block 50, then degrades to 11.0 by block 100 and oscillates 8.8–10.6 for the remaining 1900 blocks; churn reaches ever=100%, ≥4=97%, ≥8=88% — every weight is scrambled repeatedly. Starting from the exported Python-acc state gives the same regime (2.6M mass kick at block 25, brief recovery to 7.24, then degradation). Final held-out CE: **9.42 (zeroed start) / 9.48 (exported start)** vs 9.01 random — the sustained toggle drives the distribution to equipartition (−1=48.7%, 0=2.6%, +1=48.7%) and **destroys the learned structure while keeping full rank**. Contrast: on the *collapsed base model* the same operator is stable at 6.838 over 500 blocks (#10). So **toggle is a rescue operator at the saturation wall, not a stable sustained learner on an already-diverse model** — the headline 6.43/6.838 numbers are transient-window results (≤~500 blocks), and longer on-device runs must stop toggling (or anneal it off) once out of the wall.
...
**Long-run caveat (finding #12)**: on-device toggle is a transient-window method. A 2000-block continuation from the toggle checkpoint mass-kicks at block 25 (even with `--sl-reset-acc`), scrambles every weight, and degrades held-out CE to ~9.4 — reproduce with:
```bash
...
```
The 6.43/6.838 headline numbers hold in the ≤500-block window only; longer on-device runs should anneal toggle off after the wall.

**What this does not cover**: CUDA-scale scaling tests of the toggle rule, and annealing schedules for toggle (open from #12).
...
3. ~~**Ternary learning is starved by saturation**~~ → **fixed by the toggle rule** (#8–#12): 94–100% no-op flips on ±1-saturated weights disappear; toggled runs keep diversity and reach 6.43 held-out CE. ~~Left to verify: toggle stability past 1000+ steps~~ → **answered by #12: not stable** — sustained on-device toggle mass-kicks at block 25 (even from zeroed accumulators), churns every weight, and degrades held-out CE to ~9.4 by 2000 blocks. Toggle is a rescue/search operator at the wall; longer runs need to stop toggling (or anneal it off) once the wall is crossed.
