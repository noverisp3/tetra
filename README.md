...
**Long-run caveat (finding #12)**: on-device toggle is a transient-window method and **cannot be frozen by annealing** — the ±20 wall crossing is periodic (re-hits every ~25 blocks from zeroed accs) and destructive (the post-kick state evals at 12.43 held-out). Reproduce with:
```bash
...
```
The 6.43/6.838 headline numbers hold only in the transient window: Python 250–500 toggle steps (6.43) or the one-time base-model rescue (6.838, 500 blocks). The only sustained toggle recipe found is `--toggle-window N` for short diagnostics.

**What this does not cover**: CUDA-scale scaling tests of the toggle rule, and threshold/decay schedules that could make sustained toggle non-periodic (e.g. an accumulating threshold).
