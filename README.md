...
Base binary layout, shared by all versions (v5+ adds extended header fields; v6/v7 deltas documented under Discrete Learning, v8 delta next section):
...
Version deltas: **v6** = +FP32 accumulator per ternary entry + SL metadata (below). **v7** = +outlier side-channel blobs (code 11 = ±2, below). **v8** = +true-value outlier blobs (code 11 = ±2 with top-k latent values, next section).
...
- **Kernel-level parity test** (`tests/test_apply_flip_parity.py`): drives a standalone C++ harness (`inference/apply_flip_driver.cpp`, wraps `tetra.h::apply_bit_flips`, built on the fly with g++) against `quantization.apply_bit_flips` on identical packed/acc/blob inputs. Covers v6 strict parity (packed + accumulator bytes), v7 strict parity incl. promote/demote and the rebuilt blob, v7 adaptive-k·RMS threshold (with an fp64-vs-fp32 parked-acc tolerance band), and the v8 freeze invariant (code-11 top-k outliers untouched — the `is_std` boundary `|w| < 1.5f` vs `≤ 1.5` fix). 40/40 cases, run with `pytest tests/test_apply_flip_parity.py`.
...
## ERC — Echo Residual Committer (STE continual learning)
...
ternary_llm/
  ...
  cli.py                    # Shared argparse helpers (data/preset + flip flags) for the 3 train scripts
  csrc/
